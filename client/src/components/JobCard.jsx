  ...
  return (
    <div className="border shadow rounded p-6">
      ...
      <div className="flex mt-4 gap-4 text-sm ">
        <button onClick={()=>{navigate(`/apply-job/${job._id}`); scrollTo(0,0)}} className="bg-blue-600 text-white rounded py-2 px-4">Apply now</button>
        <button onClick={()=>{navigate(`/apply-job/${job._id}`); scrollTo(0,0)}} className="text-gray-500 py-2 px-4 rounded border border-gray-500">Learn more</button>
      </div>
    </div>
  );
};
