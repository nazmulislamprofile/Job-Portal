        ...
        <div className="flex gap-2 mb-6 mt-3">
          {
            isEdit||userData && userData.resume===""?<>
                <label htmlFor="resumeUpload" className="flex items-center">
                    <p className="bg-blue-100 text-blue-600 px-4 py-2 rounded-lg mr-2">{resume?resume.name:"resume"}</p>
            ...
            <tbody>
                {
                    userApplications.map((job,index)=>true?(
                        <tr key={index}>
                            ...
                            <td className="py-3 px-4 border-b"><span className={`${job.status==="Accepted"?"bg-green-100":job.status==="Rejected"?"bg-red-100":'bg-blue-100'} px-4 py-1.5 rounded`}>
                            {job.status}
                            </span></td>
                        </tr>
                    ):(null))
